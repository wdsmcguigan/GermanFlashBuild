...
export function FlashcardStudy({
  ...
  const [helpUsedForId, setHelpUsedForId] = useState<string | null>(null);
  ...
  const cleanConjugation = (val: string | undefined | null) => {
    if (!val) return "—";
    return val.replace(
      /^(er\/sie\/es|er|sie|es|ich|du|wir|ihr|sie\/Sie)\s+/i,
      "",
    );
  };
  ...
  const skipBackward = () => {
    setFlipped(false);
    setOverrideIndex(
      (prev) => (prev - 1 + sortedWords.length) % sortedWords.length,
    );
  };
  ...
  const handleMastered = () => {
    if (!activeWord) return;
    if (helpUsedForId === activeWord.id) return;
    onUpdateWord(activeWord.id, {
      level: 5,
  ...
  if (words.length === 0) {
      ...
      return (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center w-full">
          ...
          <p className="text-[#8E8E80] mt-2 max-w-sm mx-auto">
            You don't have any words marked as "{studyFilter}" yet.
          </p>
          {onClearFilter && (
            <button
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center w-full">
        ...
        <p className="text-[#8E8E80] mt-2">
          Head over to the Builder to add some words first.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className={`flex-1 flex flex-col items-center justify-start pb-24 w-full h-full lg:min-h-0 min-h-[400px] ${
        studyFilter && studyFilter !== "all" ? "pt-4 md:pt-8" : "pt-12 md:pt-20"
      }`}
    >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#4A4A40] font-medium">
              ...
              <span className="font-bold capitalize">
                {studyFilter === "new" ? "new words" : studyFilter}
              </span>{" "}
              subset ({words.length} {words.length === 1 ? "word" : "words"})
            </span>
          </div>
          {onClearFilter && (
            <button
      ...
      <div
        className="w-full max-w-lg mb-6 lg:mb-8 lg:mt-0"
      ...
      >
        ...
        <HintHelper
          ...
          onHelpUsed={() => setHelpUsedForId(activeWord.id)}
        >
            ...
            >
              ...
              <div className="absolute top-4 left-4 right-4 flex justify-between items-center pointer-events-none z-20">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setEditingWordId(activeWord.id);
                  }}
                ...
                <div className="relative pointer-events-auto">
                  <button
                    ...
                    disabled={helpUsedForId === activeWord.id}
                    title={
                      helpUsedForId === activeWord.id
                        ? "Disabled — a hint was used on this card"
                        : "Mark as mastered"
                    }
                    className={`p-2 rounded-full transition-colors ${helpUsedForId === activeWord.id ? "opacity-70 cursor-not-allowed" : "cursor-pointer"} ${helpUsedForId === activeWord.id ? (flipped ? "bg-white/10 text-white/30" : "bg-neutral-100 text-neutral-400") : flipped ? "bg-green-400/20 text-green-300 hover:bg-green-400/30" : "bg-green-500/10 text-green-600 hover:bg-green-500/20"}`}
                  >
                    <CheckCircle2 className="w-5 h-5" />
                  </button>
                  {helpUsedForId === activeWord.id && (
                    <span
                      className={`absolute top-full right-0 mt-1 whitespace-nowrap text-[8px] tracking-widest uppercase font-bold ${flipped ? "text-white/40" : "text-neutral-400"}`}
                    >
                      hint used
                    </span>
                  )}
              ...
              <div className="absolute bottom-4 left-4 flex justify-between items-center pointer-events-none z-20">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onUpdateWord(activeWord.id, { pinned: !activeWord.pinned });
                  }}
                  title={activeWord.pinned ? "Unpin word" : "Pin word"}
                  className={`pointer-events-auto p-2 rounded-full transition-colors cursor-pointer ${activeWord.pinned ? "text-pink-500 bg-pink-500/10 hover:bg-pink-500/20" : flipped ? "text-white/40 hover:text-white/80 hover:bg-white/10" : "text-[#8E8E80] hover:text-[#5A5A40] hover:bg-black/5"}`}
                >
                  <Pin
                    className={`w-4 h-4 ${activeWord.pinned ? "fill-current" : ""}`}
                  />
                </button>
              </div>
              ...
              <div className="absolute top-0 left-0 w-full h-1.5 flex bg-[#F9F9F4]">
                <div
                  ...
                  style={{ width: `${(activeWord.level / 5) * 100}%` }}
                />
              </div>
              ...
              {!flipped ? (
                    ...
                    <div className="flex items-center justify-center gap-2 mb-4 mt-8">
                      ...
                      {(activeWord.cefrLevel || activeWord.theme) && (
                        <span className="text-[10px] text-[#8E8E80] opacity-80 font-medium">
                          {activeWord.cefrLevel ? activeWord.cefrLevel : ""}
                          {activeWord.cefrLevel && activeWord.theme
                            ? " - "
                            : ""}
                          {activeWord.theme ? activeWord.theme : ""}
                        </span>
                      )}
                    </div>
                  )}
                  <span className="text-[10px] uppercase tracking-widest text-[#8E8E80] mb-2 font-bold">
                    German
                  </span>
                  <h3
                    className="text-3xl sm:text-4xl lg:text-5xl font-serif font-light text-[#2A2A20] leading-tight"
                    style={{ fontFamily: "Georgia, serif" }}
                  >
                    {displayGerman}
                  </h3>
                ...
                <div className="w-full flex flex-col items-center justify-center mt-6">
                  <span className="text-[10px] uppercase tracking-widest opacity-60 mb-2 font-bold text-white/80">
                    English
                  </span>
                  <h3
                    className="text-2xl sm:text-3xl font-serif text-white mb-4 leading-tight"
                    style={{ fontFamily: "Georgia, serif" }}
                  >
                    {activeWord.english}
                  </h3>
                  ...
                  {activeWord.wordType === "verb" &&
                    (activeWord.present ||
                      activeWord.preterite ||
                      activeWord.perfect) && (
                      <div className="mt-2 mb-4 p-3 bg-white/10 rounded-2xl w-full text-left font-sans text-white/95 border border-white/5 shadow-inner">
                        ...
                        <div className="grid grid-cols-4 gap-1 text-[10px] sm:text-[11px] md:text-xs text-center font-serif leading-tight">
                          <div
                            className="break-words font-bold italic"
                            style={{ fontFamily: "Georgia, serif" }}
                          >
                            {activeWord.german}
                          </div>
                          <div className="break-words">
                            {cleanConjugation(activeWord.present)}
                          </div>
                          <div className="break-words">
                            {cleanConjugation(activeWord.preterite)}
                          </div>
                          <div className="break-words">
                            {cleanConjugation(activeWord.perfect)}
                          </div>
                        </div>
                    ...
                    <div className="mt-2 pt-3 border-t border-opacity-20 border-white w-full text-white flex flex-col items-center">
                      {(() => {
                        ...
                        if (parts.length >= 2) {
                          return (
                            <>
                              <p className="text-[13px] sm:text-sm font-serif italic opacity-95 mb-1 max-w-xs leading-relaxed text-center">
                                "{parts[0].trim()}"
                              </p>
                              <p className="text-[11px] sm:text-xs font-sans opacity-75 max-w-xs leading-relaxed text-center">
                        ...
                        return (
                          <p className="text-[13px] sm:text-sm italic opacity-85 leading-relaxed max-w-xs mx-auto font-serif text-center">
                            "{example}"
                          </p>
                        );
                      })()}
                  ...
                  <p className="mt-6 text-[10px] text-white/50 flex items-center gap-1.5 italic">
                    <RotateCcw className="w-3 h-3" /> Tap to toggle back to
                    German
                  </p>
                </div>
              )}
      ...
      <div className="w-full max-w-lg flex flex-col gap-4">
        ...
        <div className="flex items-center justify-center gap-6">
          <button
            ...
            className="p-2 text-[#8E8E80] hover:text-[#5A5A40] hover:bg-white rounded-full transition-colors shadow-sm bg-transparent"
            title="Previous word"
          >
          ...
          <button
            onClick={skipRandom}
            className="flex items-center gap-2 px-4 py-2 text-[#8E8E80] hover:text-[#5A5A40] text-[10px] font-bold uppercase tracking-widest hover:bg-white rounded-full transition-colors shadow-sm bg-transparent"
            title="Random word"
          >
