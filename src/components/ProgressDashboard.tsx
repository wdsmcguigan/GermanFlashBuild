...
interface ProgressDashboardProps {
  words: VocabWord[];
  onStartTest: (
    category: "all" | "new" | "learning" | "mastered" | "pinned",
  ) => void;
}
...
export function ProgressDashboard({
  ...
  const [expandedCategory, setExpandedCategory] = useState<
    "New" | "Learning" | "Mastered" | "Pinned" | null
  >(null);
  ...
  const expandedWords = useMemo(() => {
    ...
    if (expandedCategory === "New")
      return words.filter((w) => (w.level || 1) <= 1);
    if (expandedCategory === "Learning")
      return words.filter((w) => (w.level || 1) > 1 && (w.level || 1) < 5);
    if (expandedCategory === "Mastered")
      return words.filter((w) => (w.level || 1) >= 5);
    if (expandedCategory === "Pinned") return words.filter((w) => w.pinned);
    return [];
  ...
  const stats = useMemo(() => {
    ...
    const pinnedRate =
      totalCount > 0 ? Math.round((pinnedCount / totalCount) * 100) : 0;
  ...
  const cefrStats = useMemo(() => {
    ...
    return {
      distribution: levels.map((l) => ({
        ...
        percentage:
          totalCount > 0 ? Math.round((counts[l].total / totalCount) * 100) : 0,
      })),
  ...
  const categoryStats = useMemo(() => {
    ...
    return Object.entries(counts)
      .map(([name, data]) => ({
        ...
        percentage:
          totalCount > 0 ? Math.round((data.total / totalCount) * 100) : 0,
      }))
  ...
  if (totalCount === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center w-full">
        ...
        <h2
          className="text-xl font-serif text-[#2A2A20]"
          style={{ fontFamily: "Georgia, serif" }}
        >
          No progress tracked yet
        </h2>
        <p className="text-[#8E8E80] mt-2 max-w-sm mx-auto">
          Add vocabulary words and practice with flashcards to build your
          learning profile.
        </p>
      </div>
    );
  ...
  const strokeDashoffset =
    circumference - (stats.masteryRate / 100) * circumference;
  ...
  return (
    <div className="flex-1 overflow-y-auto w-full max-w-4xl mx-auto px-4 py-2 lg:py-6 space-y-6 custom-scrollbar">
      ...
      <div className="text-center sm:text-left space-y-1">
        <h2
          className="text-2xl font-serif text-[#5A5A40] italic font-normal"
          style={{ fontFamily: "Georgia, serif" }}
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-full">
        {/* Total Deck Size */}
        <div className="relative bg-white border border-[#E0E0D5] rounded-3xl p-5 shadow-sm flex flex-col justify-between items-start gap-4 col-span-2 md:col-span-1">
          <div className="w-12 h-12 bg-[#F5F5F0] rounded-2xl flex items-center justify-center text-[#5A5A40] shrink-0">
            <BookOpen className="w-6 h-6" />
          </div>
          <div>
            <span className="text-[10px] text-[#8E8E80] uppercase font-bold tracking-widest block">
              Total Deck
            </span>
            <span
              className="text-2xl font-serif font-bold text-[#2A2A20]"
              style={{ fontFamily: "Georgia, serif" }}
            >
              {totalCount}
            </span>
            <span className="text-[10px] text-[#8E8E80] block mt-0.5">
              words practicing
            </span>
          </div>
          {totalCount > 0 && (
        ...
        <div
          onClick={(e) => {
            e.stopPropagation();
            setExpandedCategory(expandedCategory === "New" ? null : "New");
          }}
          className={`relative metric-button border rounded-3xl p-5 shadow-sm flex flex-col justify-between items-start gap-4 cursor-pointer transition-colors ${expandedCategory === "New" ? "bg-[#F9F9F4] border-[#D0D0C5]" : "bg-white border-[#E0E0D5] hover:border-[#D0D0C5] hover:bg-[#F9F9F4]"}`}
        >
        ...
        <div
          onClick={(e) => {
            e.stopPropagation();
            setExpandedCategory(
              expandedCategory === "Learning" ? null : "Learning",
            );
          }}
          className={`relative metric-button border rounded-3xl p-5 shadow-sm flex flex-col justify-between items-start gap-4 cursor-pointer transition-colors ${expandedCategory === "Learning" ? "bg-[#F9F9F4] border-[#D0D0C5]" : "bg-white border-[#E0E0D5] hover:border-[#D0D0C5] hover:bg-[#F9F9F4]"}`}
        >
          ...
          <div className="pointer-events-none">
            ...
            <span
              className="text-2xl font-serif font-bold text-amber-600"
              style={{ fontFamily: "Georgia, serif" }}
            >
              {stats.learningCount}
            </span>
            <span className="text-[10px] text-[#8E8E80] block mt-0.5">
              {stats.learningRate}% in active rotation
            </span>
          </div>
        ...
        <div
          onClick={(e) => {
            e.stopPropagation();
            setExpandedCategory(
              expandedCategory === "Mastered" ? null : "Mastered",
            );
          }}
          className={`relative metric-button border rounded-3xl p-5 shadow-sm flex flex-col justify-between items-start gap-4 cursor-pointer transition-colors ${expandedCategory === "Mastered" ? "bg-[#F9F9F4] border-[#D0D0C5]" : "bg-white border-[#E0E0D5] hover:border-[#D0D0C5] hover:bg-[#F9F9F4]"}`}
        >
          ...
          <div className="pointer-events-none">
            ...
            <span
              className="text-2xl font-serif font-bold text-green-700"
              style={{ fontFamily: "Georgia, serif" }}
            >
              {stats.masteredCount}
            </span>
            <span className="text-[10px] text-[#8E8E80] block mt-0.5">
              {stats.masteryRate}% at top durability
            </span>
          </div>
        ...
        <div
          onClick={(e) => {
            e.stopPropagation();
            setExpandedCategory(
              expandedCategory === "Pinned" ? null : "Pinned",
            );
          }}
          className={`relative metric-button border rounded-3xl p-5 shadow-sm flex flex-col justify-between items-start gap-4 cursor-pointer transition-colors ${expandedCategory === "Pinned" ? "bg-[#F9F9F4] border-[#D0D0C5]" : "bg-white border-[#E0E0D5] hover:border-[#D0D0C5] hover:bg-[#F9F9F4]"}`}
        >
          <div className="w-12 h-12 bg-pink-500/10 rounded-2xl flex items-center justify-center text-pink-600 shrink-0 pointer-events-none">
          ...
          {stats.pinnedCount > 0 && (
            <button
              ...
              className="absolute top-4 right-4 px-2.5 py-1 bg-pink-500/10 hover:bg-pink-600 hover:text-white text-pink-700 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all flex items-center gap-1 border border-pink-500/20 shadow-sm"
              title="Test Pinned words"
            >
              <Play className="w-3 h-3 fill-current" />
              <span>Test</span>
            </button>
        ...
        <div className="list-container bg-white border border-[#E0E0D5] rounded-[32px] overflow-hidden shadow-sm animate-fade-in relative z-10 w-full mb-6">
          ...
          <div className="max-h-[400px] overflow-y-auto px-2 py-4 space-y-1 custom-scrollbar w-full">
            {expandedWords.length === 0 ? (
              <div
                className="p-8 text-center text-[#8E8E80] text-sm italic font-serif"
                style={{ fontFamily: "Georgia, serif" }}
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div className="bg-white border border-[#E0E0D5] rounded-[32px] p-6 shadow-sm md:col-span-2 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center justify-center">
              <span
                className="text-3xl font-serif font-bold text-[#2A2A20]"
                style={{ fontFamily: "Georgia, serif" }}
              >
                {stats.masteryRate}%
              </span>
              <span className="text-[8px] text-[#8E8E80] font-mono tracking-wider uppercase font-bold mt-0.5">
                Retained
              </span>
            </div>
          </div>
        ...
        <div className="bg-white border border-[#E0E0D5] rounded-[32px] p-6 shadow-sm md:col-span-3 flex flex-col justify-between">
          <div>
            <span className="text-[10px] text-[#8E8E80] uppercase font-bold tracking-widest block mb-4">
              Deck Progression Share
            </span>
            ...
            <p className="text-xs text-[#8E8E80] mt-3 leading-relaxed">
              Words are shifted between boxes based on your review results.
              Standard intervals automatically space flashcard recurrences
              relative to each box level.
            </p>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-[#F5F5F0] grid grid-cols-3 text-center gap-2">
            <div className="p-2.5 bg-[#FDFDFB] rounded-2xl border border-[#F5F5F0]">
              <span className="text-[9px] font-mono uppercase text-[#8E8E80] block font-bold">
                New
              </span>
              <span
                className="text-base font-serif font-bold text-[#4A4A40] inline-flex items-center gap-1"
                style={{ fontFamily: "Georgia, serif" }}
              >
                1{" "}
              ...
              <span className="text-[8px] text-[#8E8E80] block mt-0.5">
                interval
              </span>
            </div>
            <div className="p-2.5 bg-[#FDFDFB] rounded-2xl border border-[#F5F5F0]">
            ...
            <div className="p-2.5 bg-[#FDFDFB] rounded-2xl border border-[#F5F5F0]">
              ...
              <span
                className="text-base font-serif font-bold text-green-700 inline-flex items-center gap-1"
                style={{ fontFamily: "Georgia, serif" }}
              >
                30{" "}
                <span className="text-[10px] font-sans font-normal opacity-60">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-[#E0E0D5] rounded-[32px] p-6 shadow-sm space-y-4">
          <div className="justify-between flex items-center">
            ...
            <span className="text-[9px] font-mono bg-[#F5F5F0] text-[#5A5A40] px-2 py-0.5 rounded uppercase font-bold">
          ...
          {!cefrStats.hasCefrData ? (
            <div className="py-8 flex flex-col items-center justify-center text-[#E0E0D5] text-center border border-dashed border-[#F5F5F0] rounded-2xl">
              ...
              <p
                className="text-xs font-serif italic text-[#8E8E80]"
                style={{ fontFamily: "Georgia, serif" }}
              >
                Add vocabulary with CEFR levels (e.g., A1, B2) to populate this
                matrix.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {cefrStats.distribution.map((item) => {
                const ratio =
                  item.total > 0 ? (item.mastered / item.total) * 100 : 0;
                return (
                  <div key={item.level} className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-[#2A2A20]">
                        {item.level}
                      </span>
                    ...
                    <div className="w-full h-2 bg-[#F5F5F0] rounded-full overflow-hidden border border-[#E0E0D5]/40 relative">
                      {item.total > 0 ? (
                        <>
                          <div
                            style={{
                              width: `${(item.total / totalCount) * 100}%`,
                            }}
                            className="absolute top-0 left-0 h-full bg-[#E0E0D5]"
                          />
                          <div
                            style={{
                              width: `${(item.mastered / totalCount) * 100}%`,
                            }}
        ...
        <div className="bg-white border border-[#E0E0D5] rounded-[32px] p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-4">
            {categoryStats.length === 0 ? (
              <p
                className="text-xs italic text-[#8E8E80] font-serif"
                style={{ fontFamily: "Georgia, serif" }}
              >
              ...
              categoryStats.map((cat) => {
                const ratio =
                  cat.total > 0
                    ? Math.round((cat.mastered / cat.total) * 100)
                    : 0;
                return (
                  <div
                    key={cat.name}
                    className="flex items-center justify-between border-b border-[#F5F5F0] pb-2 last:border-b-0 last:pb-0"
                  >
                    <div>
                      <span className="text-xs font-semibold text-[#2A2A20] block">
                        {cat.name}
                      </span>
                      <span className="text-[10px] text-[#8E8E80]">
                        {cat.total} words added
                      </span>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-serif font-bold text-[#5A5A40] block">
                        {ratio}% Mastered
                      </span>
